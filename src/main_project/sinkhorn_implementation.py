...
if __name__ == "__main__":
    ...
    for i in range(n_points):
        ...
        ax.plot(path[:, 0], path[:, 1], color="gray", alpha=0.4, linewidth=1.0, zorder=1)
        ...
        for fraction in intermediate_fractions:
            ...
            # Mark on plot
            ax.scatter(*intermediate_latent, color="green", s=30, alpha=0.6, zorder=2, marker="s")
        ...
        ax.scatter(
            *x0, color="steelblue", edgecolors="black", s=60, zorder=3, label="Digit 0 (source)" if i == 0 else ""
        )
        ax.scatter(
            *x1, color="coral", edgecolors="black", s=60, zorder=3, label="E[y|x*] digit 1 (target)" if i == 0 else ""
        )
