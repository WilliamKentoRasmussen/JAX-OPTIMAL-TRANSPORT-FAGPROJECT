...
def compute_bo_metrics(model, val_loader, source_class=0, target_class=1):
    ...
    mmd_latent = float(MMD(
        jnp.array(source_z[:n_min]),
        jnp.array(target_z[:n_min]),
        kernel="rbf"
    ))
...
def objective(trial):
    ...
    arch_name  = trial.suggest_categorical("arch", ["small2", "small", "medium", "large", "large2", "medium2"]) #try different architectures
    hidden_dim = arch_presets[arch_name]
    lr         = trial.suggest_float("lr",        1e-4, 1e-2, log=True) #try different learning rates in the log domain within the domain
    lambda_l2  = trial.suggest_float("lambda_l2", 1e-5, 1e-1, log=True) #try regulariztion rates in the log domain within the domain
