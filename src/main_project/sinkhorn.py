...
def sinkhorn(x: torch.Tensor, y: torch.Tensor, p: float = 2,
    ...
    #Bruger euclidean distance til lave cost matricen M
    if p == 1:
        M_ij = ((x_i - y_j) ** p).abs().sum(dim=2) # [n, m] 
    else:
        M_ij = ((x_i - y_j) ** p).sum(dim=2) ** (1.0 / p)  # [n, m]

    # Weights [n,] and [m,]. Hvis massen af hvertpoint ikke er givet, vil massen blive uniformt fordelt mellme punkterne.
    ...
    #Tilføjer entropy regulariserede termer 
    ...
    if verbose: # Tilføjer tqdm loading bar
    ...
    for _ in pbar:
        ...
        if max_err_u < stop_thresh and max_err_v < stop_thresh: #Stops når enten u eller v konvergerer
            break
...
if __name__ == "__main__":
    ...
    print("Wasserstein distance:", distance.item())
    print("x -> y correspondences:", corr_x_to_y)
    print("y -> x correspondences:", corr_y_to_x)
