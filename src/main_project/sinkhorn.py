...
def run_sinkhorn_by_model(model):
    ...
    T, u, v, iter = sinkhorn_log(C=C, s=s, d=d, gamma=gamma, max_iters=1000, stop_thresh=stop_threshold, verbose=True)
...
def save_sinkhorn_transformation(model, save = True):
    ...
    n_points = max(MAX_POINTS, len(latent_source))
    ...
    if save: 
        np.save(f"data/{model}/intermediate_images.npy", np.array(intermediate_images))
        np.save(f"data/{model}/original_images.npy", np.array(source_images))
        np.save(f"data/{model}/expected_target_images.npy", np.array(expected_target_images))
...
def main():
    for dim in MODELS_DIM: 
        ...
        save_sinkhorn_transformation(model = model, save =True)
