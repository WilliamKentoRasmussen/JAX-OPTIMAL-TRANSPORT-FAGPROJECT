...
def sinkhorn_simple(
    ...
    for i in range(max_iters):
        ...
        if (
            ...
            and jnp.max(jnp.abs(v_prev - v)) < stop_thresh
        ):
            if verbose:
...
if __name__ == "__main__":
    ...
    ax.scatter(src_np, np.zeros_like(src_np), s=s * 2000,
               label="Source support", alpha=0.7, zorder=3)
    ax.scatter(tgt_np, np.ones_like(tgt_np), s=d * 2000,
               label="Target support", alpha=0.7, zorder=3)
    ax.scatter([new_np],  [0], marker="*", s=300, color="red",
               label="New point (source)", zorder=4)
    ax.scatter([trans_np], [1], marker="*", s=300, color="darkred",
               label="Transported point (target)", zorder=4)
    ax.annotate("", xy=(trans_np, 1), xytext=(new_np, 0),
